...
Levantamento obtido via `DbExplorerSP.executeQuery` no banco Oracle do Sankhya (ambiente de homologação).
...
|---|---|---|---|---|---|
| 1 | `NUREL` | NUMBER | 10,0 | N | Número único do registro de chamada/relacionamento (PK) |
| 2 | `CODPARC` | NUMBER | 10,0 | N | Código do parceiro (FK → TGFPAR) |
| 3 | `DHCHAMADA` | DATE | 7 | N | Data e hora da chamada/contato |
| 4 | `CODPROD` | NUMBER | 10,0 | Y | Código do produto relacionado |
| 5 | `TIPCHAM` | VARCHAR2 | 1 | Y | Tipo de chamada (domínio) |
| 6 | `CODCONTATO` | NUMBER | 5,0 | Y | Código do contato no parceiro |
| 7 | `CODHIST` | NUMBER | 5,0 | Y | Código do histórico (motivo) |
| 8 | `COMENTARIOS` | VARCHAR2 | 300 | Y | Comentários da chamada |
| 9 | `DHPROXCHAM` | DATE | 7 | Y | Data e hora da próxima chamada (agendamento) |
| 10 | `PENDENTE` | VARCHAR2 | 1 | N | Indica se o atendimento está pendente (S/N) |
| 11 | `CODUSU` | NUMBER | 5,0 | N | Código do usuário responsável |
| 12 | `COMENTARIOS2` | VARCHAR2 | 250 | Y | Comentários adicionais |
| 13 | `CODVEND` | NUMBER | 5,0 | Y | Código do vendedor |
| 14 | `SITUACAO` | VARCHAR2 | 1 | N | Situação do registro (domínio) |
| 15 | `CODATENDENTE` | NUMBER | 5,0 | N | Código do atendente |
| 16 | `DTALTER` | DATE | 7 | N | Data de alteração |
| 17 | `NUMOS` | NUMBER | 10,0 | Y | Número da ordem de serviço |
| 18 | `TEMPPREVISTO` | DATE | 7 | Y | Tempo previsto |
| 19 | `NUAVISO` | NUMBER | 10,0 | Y | Número do aviso |
| 20 | `AD_TIPCHAMADA` | VARCHAR2 | 10 | Y | **AD:** Tipo de chamada (customizado) |
| 21 | `AD_HRCHECKOUT` | DATE | 7 | Y | **AD:** Hora de checkout |
| 22 | `AD_HRCHECKIN` | DATE | 7 | Y | **AD:** Hora de checkin |
| 23 | `AD_HISTORICO` | VARCHAR2 | 10 | Y | **AD:** Código do histórico (customizado) |
...
| 27 | `AD_HISTCOBRA` | VARCHAR2 | 10 | Y | **AD:** Histórico de cobrança |
| 28 | `AD_MSG` | VARCHAR2 | 4000 | Y | **AD:** Mensagem da cobrança |

### Campos relevantes para cobrança
- `NUREL` — identificador único do contato
- `CODPARC` — vincula o contato ao cliente/parceiro
- `DHCHAMADA` — quando o contato foi feito
- `DHPROXCHAM` — agendamento do próximo contato
- `PENDENTE` — se ainda precisa de retorno (S/N)
- `AD_TIPCHAMADA` / `AD_TIPO` — tipo do contato (ex: WhatsApp, telefone, e-mail)
- `AD_HISTCOBRA` — histórico de cobrança
- `AD_MSG` — mensagem enviada ao cliente (até 4000 chars)
- `COMENTARIOS` / `COMENTARIOS2` — observações do atendente
...
| # | Campo | Tipo | Tam/Prec | Null | Descrição |
|---|---|---|---|---|---|
| 1 | `NUFIN` | NUMBER | 10,0 | N | **PK** — Número único do título financeiro |
| 2 | `CODEMP` | NUMBER | 5,0 | N | Código da empresa |
| 3 | `NUMNOTA` | NUMBER | 10,0 | N | Número da nota fiscal |
| 4 | `SERIENOTA` | VARCHAR2 | 3 | Y | Série da nota |
| 5 | `DTNEG` | DATE | 7 | N | Data da negociação (emissão do título) |
| 6 | `DESDOBRAMENTO` | VARCHAR2 | 3 | Y | Desdobramento (identifica a parcela: A, B, C...) |
| 7 | `DHMOV` | DATE | 7 | N | Data e hora da movimentação |
| 8 | `DTVENCINIC` | DATE | 7 | Y | Data do primeiro vencimento |
| 9 | `DTVENC` | DATE | 7 | Y | **Data de vencimento** do título |
| 10 | `DHBAIXA` | DATE | 7 | Y | Data/hora da baixa (null = em aberto) |
| 13 | `CODPARC` | NUMBER | 10,0 | N | Código do parceiro (FK → TGFPAR) |
| 14 | `CODTIPOPER` | NUMBER | 5,0 | N | Código do tipo de operação |
| 16 | `CODBCO` | NUMBER | 5,0 | N | Código do banco |
| 18 | `CODNAT` | NUMBER | 10,0 | N | Código da natureza financeira |
| 21 | `CODVEND` | NUMBER | 5,0 | N | Código do vendedor |
| 23 | `CODTIPTIT` | NUMBER | 5,0 | N | Código do tipo de título |
| 24 | `NUMDUPL` | NUMBER | 10,0 | Y | Número da duplicata |
| 26 | `NOSSONUM` | VARCHAR2 | 12 | Y | Nosso número (boleto bancário) |
| 27 | `HISTORICO` | VARCHAR2 | 255 | Y | Histórico/descrição do título |
| 28 | `VLRDESDOB` | FLOAT | 126 | N | **Valor original** do título (desdobrado) |
| 29 | `VLRVENDOR` | FLOAT | 126 | N | Valor vendor |
| 35 | `VLRDESC` | FLOAT | 126 | N | Valor de desconto |
| 36 | `VLRMULTA` | FLOAT | 126 | N | Valor de multa |
| 39 | `VLRJURO` | FLOAT | 126 | N | Valor de juros |
| 46 | `VLRBAIXA` | FLOAT | 126 | N | **Valor baixado** (valor já pago/recebido) |
| 49 | `RECDESP` | NUMBER | 5,0 | N | **1 = A Receber** / **-1 = A Pagar** |
| 50 | `PROVISAO` | VARCHAR2 | 1 | N | Se é provisão (S/N) |
| 51 | `ORIGEM` | VARCHAR2 | 1 | N | Origem do título |
| 53 | `NUNOTA` | NUMBER | 10,0 | Y | Número único da nota (FK → TGFCAB) |
| 59 | `DTENTSAI` | DATE | 7 | Y | Data de entrada/saída |
| 69 | `CODBARRA` | VARCHAR2 | 50 | Y | Código de barras do boleto |
| 73 | `CODIGOBARRA` | VARCHAR2 | 55 | Y | Código de barras completo |
| 74 | `LINHADIGITAVEL` | VARCHAR2 | 57 | Y | Linha digitável do boleto |
| 101 | `CODUSUCOBR` | NUMBER | 5,0 | Y | Código do usuário de cobrança |
| 100 | `PDD` | VARCHAR2 | 1 | Y | Provisão de débito duvidoso (S/N) |
| 165 | `CODREGUA` | NUMBER | 10,0 | Y | Código da régua de cobrança |
| 245 | `AD_DESCONTADO` | VARCHAR2 | 100 | Y | **AD:** Informação de desconto |

### 2.2 Regras de status (derivado — TGFFIN não tem campo de status direto)
...
| Campo | Descrição |
|---|---|
| `VLRDESDOB` | Valor original do título (sempre positivo) |
| `VLRBAIXA` | Valor efetivamente baixado (pago/recebido) |
| `VLRDESC` | Valor de desconto concedido |
| `VLRJURO` | Valor de juros cobrados |
| `VLRMULTA` | Valor de multa aplicada |
| `RECDESP` | Define o sentido: 1 = a receber do cliente, -1 = a pagar ao fornecedor |
...
|---|---|---|---|
| **Financeiro** | **TGFFIN** | NUFIN | Títulos financeiros (parcelas, duplicatas, contas a pagar/receber) |
| **Telefone** | **TGFTEL** | NUREL | Registro de chamadas/contatos com parceiros |
| **NFE** | **TGFNFE** | NUNOTA | XML da Nota Fiscal Eletrônica (NFE/NFC-e) para DANFE |
| CabecalhoNota | TGFCAB | NUNOTA | Cabeçalho de notas fiscais |
| Parceiro | TGFPAR | CODPARC | Clientes e fornecedores |

> **Correção importante:** O código anterior consultava `TGFCAB` (CabecalhoNota) para obter títulos financeiros. O correto é usar **TGFFIN** (Financeiro), que contém as parcelas/duplicatas com vencimento, valor, baixa, etc.
...
|---|---|---|---|---|---|
| 1 | `NUNOTA` | NUMBER | 10,0 | N | **PK** — Número único da nota (FK → TGFCAB) |
| 2 | `CHAVENFE` | VARCHAR2 | 44 | Y | **Chave de acesso** da NFE (44 dígitos) |
| 3 | `XML` | CLOB | 4000 | Y | **XML da NFE** (documento principal para DANFE) |
| 4 | `XMLPROTAUTNOT` | CLOB | 4000 | Y | XML do protocolo de autorização da nota |
| 5 | `XMLENVCLI` | CLOB | 4000 | Y | XML enviado ao cliente |
| 6 | `XMLCANC` | CLOB | 4000 | Y | XML de cancelamento |
| 7 | `XMLPROTCANC` | CLOB | 4000 | Y | XML do protocolo de cancelamento |
| 8 | `XMLENVCLICANC` | CLOB | 4000 | Y | XML de cancelamento enviado ao cliente |
| 9 | `XMLENVCARTA` | CLOB | 4000 | Y | XML de carta de correção |
| 10 | `XMLPROTAUTCARTA` | CLOB | 4000 | Y | Protocolo de autorização da carta de correção |
| 11 | `XMLENVCLICARTA` | CLOB | 4000 | Y | Carta de correção enviada ao cliente |
| 12 | `QRCODE` | VARCHAR2 | 4000 | Y | QR Code (para NFC-e — link de consulta) |
| 13 | `XMLENVEPEC` | CLOB | 4000 | Y | XML de EPEC (Evento Prévio de Emissão em Contingência) |
| 14 | `XMLPROTAUTEPEC` | CLOB | 4000 | Y | Protocolo de autorização do EPEC |
| 15 | `XMLENVCANCPRORROG` | CLOB | 4000 | Y | XML de cancelamento de prorrogação |
| 16 | `XMLENVCLICANCPRORROG` | CLOB | 4000 | Y | Cancelamento de prorrogação enviado ao cliente |
| 17 | `XMLPROTAUTCANCPRORROG` | CLOB | 4000 | Y | Protocolo de cancelamento de prorrogação |
| 18 | `XMLENVCLIPRORROG` | CLOB | 4000 | Y | Prorrogação enviada ao cliente |
| 19 | `XMLENVPRORROG` | CLOB | 4000 | Y | XML de prorrogação (Sefaz) |
| 20 | `XMLPROTAUTPRORROG` | CLOB | 4000 | Y | Protocolo de autorização de prorrogação |
| 21 | `XMLENVCLICANCCONF` | CLOB | 4000 | Y | Cancelamento de confirmação enviado ao cliente |
| 22 | `XMLENVECONF` | CLOB | 4000 | Y | XML de ECONF (Evento de Confirmação) |
| 23 | `XMLENVCANCCONF` | CLOB | 4000 | Y | XML de cancelamento de confirmação |
| 24 | `XMLPROTAUTCANCCONF` | CLOB | 4000 | Y | Protocolo de cancelamento de confirmação |
| 25 | `XMLENVCLIECONF` | CLOB | 4000 | Y | ECONF enviado ao cliente |
| 26 | `XMLPROTAUTECONF` | CLOB | 4000 | Y | Protocolo de autorização do ECONF |

### Campos relevantes para visualização de DANFE
- `NUNOTA` — vincula com `TGFFIN.NUNOTA` (cada título financeiro tem a nota origem)
- `CHAVENFE` — chave de acesso de 44 dígitos (para consulta na SEFAZ)
- `XML` — XML completo da NFE (contém emitente, destinatário, itens, totais, impostos)
- `QRCODE` — URL com QR Code (para NFC-e em varejo)
