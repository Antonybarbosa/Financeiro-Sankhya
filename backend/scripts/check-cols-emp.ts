...
async function main() {
  ...
  for (const t of ['TGFEMP', 'TSIEMP']) {
    try {
      const r = await query(token, `SELECT * FROM ${t} WHERE ROWNUM <= 1`);
      console.log(`${t}:`, r.meta.join(', '));
      if (r.rows.length) console.log(`${t} exemplo:`, r.rows[0].join(' | '));
