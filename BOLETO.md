...
> Referência técnica funcionalidade boleto Financeiro Sankhya.
...
Cada título **TGFFIN** com boleto gerado (campos `CODIGOBARRA` + `LINHADIGITAVEL` preenchidos) pode ser aberto num popup que renderiza boleto **Recibo do Sacado + Ficha de Compensação**, replicando layout nativo impressão Sankhya (arquivos `.jrxml` da instalação).

Onde abrir:

| Local | Arquivo | Ação |
|---|---|---|
| Painel do parceiro | `frontend/components/cobranca/ParceiroDetailPanel.tsx` | Botão boleto por título |
| Visão em tabela | `frontend/components/cobranca/views/TableView.tsx` | Botão boleto por título |

Mesmo componente `BoletoDocumento` usado **na tela e na impressão** (fonte única).
...
Campos TSICTA usados: `CARTEIRA`, `CODAGE`, `CONVENIO`, `DIASPROT`.
...
|---|---|---|
| `boleto_modelo.jrxml` | Modelo antigo — título "RECIBO DO PAGADOR", mini-resumo 4 colunas, valores **Times**, grade 79/21, "CIP", Instruções com "Sujeito a protesto N dias" | ❌ substituído |
...
**Como extrair estrutura de um jrxml** (Python + ElementTree, namespace `http://jasperreports.sourceforge.net/jasperreports`):
...
Largura relatório ≈ 534px. **Colunas: principal x=6..436 (430px ≈ 81,9%) + totais x=437..532 (95px ≈ 18,1%)** — mesma proporção caixa Instruções.

### Cabeçalho
- **Recibo:** [logo] + `NOME DO BANCO` + `|237-2|` + `RECIBO DO SACADO` (à direita).
- **Ficha:** [logo] + `NOME DO BANCO` + `|237-2|` + **linha digitável** à direita.
- Logo: nativo usa imagem (`logos/bradesco.jpg`); no app placeholder com iniciais banco.

### Recibo (coluna principal)
1. **Caixa vazia** (12px) + **Beneficiário/CNPJ/CPF/endereço** (célula 2 linhas com endereço cedente embaixo).
2. Direita: **Vencimento** + **Agência/Conta** empilhados (bordas internas alturas diferentes, como no jrxml).
...
| Linha | Células |
|---|---|
| Dados | Data Documento · Número Documento · **Esp.Doc. (DM, SEM borda)** · Aceite (N) · Data Processamento (hoje) · Nosso Número |
| Uso do Banco | Uso Banco (vazio) · Carteira (2 dígitos) · Espécie (R$) · Quantidade (vazio) · Valor (vazio) · **(=) Valor Documento** |
...
|---|---|---|
| **Nosso Número** | `carteira(2)/NOSSONUM[:-1]-DV` — **NÃO corta prefixo 3 dígitos** (`000000109099` → `09/00000010909-9`) ⚠️ | `formatarNossoNumeroBradesco` (BoletoViewer) |
...
| **Totais** | jrxml só preenche Valor Documento + Desconto; app também preenche **Mora/Multa** + **Valor Cobrado** (calculados) | `LinhaTotais` |
| **Banco 2 dígitos** | Sankhya grava `33` (Santander) sem zero → normalizar com `padStart(3)` | `nomeBanco`/`codigoBancoDv` (lib/boleto.ts) |
| Fonte | Tudo **SansSerif**, preto no branco, labels ~7-8px, valores ~8px (escalados no app) | ST no BoletoViewer |

> ⚠️ **Prefixo Nosso Número:** modelo Bradesco exibe NOSSONUM inteiro (`09/00000010909-9`). Modelo antigo cortava 3 primeiros dígitos (`9/00010909-9`). **Decisão pendente** — se preferir sem prefixo, é uma linha em `formatarNossoNumeroBradesco`.
...
| `Boleto_Bradesco.jrxml` / `boleto_modelo.jrxml` | Modelos nativos referência (raiz projeto) |
...
| # | Pendência | Detalhe |
|---|---|---|
| 7.1 | **Prefixo "000" do Nosso Número** | Decidir entre `09/00000010909-9` (fiel jrxml) e `09/00010909-9` (sem prefixo, como modelo antigo) |
| 7.2 | **Logo do banco** | Placeholder atual = iniciais banco; validar se serve ou trocar por imagem/abreviação oficial |
| 7.3 | **CODCTABENEF vs CONVENIO** | jrxml usa `CTA.CODCTABENEF` pra conta; testamos `CONVENIO` (funciona). Teste via script deu 404 por bug no script (`check-tsicta2.ts` usa endpoint errado) — revisitar com padrão do `check-tsicta.ts` |
| 7.4 | **Modelo por banco** | Hoje layout único (Bradesco) pra todos bancos. Opção: usar modelo Bradesco só pra CODBCO 237 e outro (antigo) pros demais |
| 7.5 | **Hardcodes do jrxml** | `Esp.Doc.=DM` e `Aceite=N` são textos fixos modelo; Nº documento com 6 dígitos idem |
| 7.6 | **DIASPROT** | Modelo Bradesco usa texto fixo "10º DIA" do SPC (não usa `DIASPROT`); modelo antigo usava `DIASPROT` no "Sujeito a protesto N dias" |
...
No app real não há esse problema: backend serializa `Date` em ISO completo (com hora).

---

## 9. Histórico resumido da implementação

1. **Planejamento** — levantamento dados disponíveis (TGFFIN, TSICTA, TSIEMP) via scripts `backend/scripts/check-*`.
2. **1ª versão** — réplica `boleto_modelo.jrxml`: RECIBO DO PAGADOR, mini-resumo com CNPJ/CPF, Times, grade 81% (alinhada à caixa Instruções a pedido usuário), totais sem "R$", ficha com "SUJEITO A PROTESTO.".
3. **Refinamentos** — nome banco (código `33` → Santander), gap "RECIBO DO PAGADOR" (1px), linha "Até o vencimento..." fundida na célula Local Pagamento (não existe linha separada no jrxml).
